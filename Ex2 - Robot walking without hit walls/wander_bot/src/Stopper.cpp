...
Stopper::Stopper(double forward_speed)
{
	forwardSpeed = forward_speed;
    keepMoving = true;

    // Advertise a new publisher for the robot's velocity command topic
    commandPub = nh.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/teleop", 10);

    // Subscribe to the simulated robot's laser scan topic
    laserSub = nh.subscribe("scan", 1, &Stopper::scanCallback, this);
}
